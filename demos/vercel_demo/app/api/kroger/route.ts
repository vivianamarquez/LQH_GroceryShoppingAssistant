...
export async function POST(request: Request) {
  try {
    ...
    if (body.action === 'search') {
      try {
        if (typeof body.zip !== 'string' || !Array.isArray(body.items) || !body.items.length || body.items.length > 20) throw new Error();
        parseResult(JSON.stringify({ title: 'Search', line_items: body.items }));
      } catch {
        return Response.json({ error: 'Provide a ZIP and up to 20 valid grocery items.' }, { status: 400 });
      }
      return Response.json(
